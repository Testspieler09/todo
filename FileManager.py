...
class FileManager:
    ...
    def __init__(self, path_to_file: str) -> None:
        if not exists(path_to_file):
            raise Exception("File not found in given directory.")
    ...
    def for_new_file(cls, path_with_filename_and_extension: str) -> None:
        ...
        with open(path_with_filename_and_extension, "w") as f:
            f.write('{"labels":[], "order of tasks in group":{}, "tasks":{}}')
...
class DataManager:
    ...
    def add_new_label_or_group(self, type_of_value: str, name: str) -> None:
        match type_of_value:
            ...
            case "g":
                if name in self.data["order of tasks in group"].keys():
                    return
    ...
    def rename(self, type_of_value: str, new_name: str, old_name: str) -> None:
        match type_of_value.lower():
            case "g":
                if old_name not in self.data["order of tasks in group"].keys():
                    return
            ...
            case "l":
                if old_name not in self.data["labels"]:
                    return
    ...
    def change_order_tasks_global_insertion(self, task_hash: str, new_idx: int) -> None:
        if task_hash not in self.data["tasks"]:
            return
    ...
    def change_order_of_steps_insertion(
        self, task_hash: str, step_hash: str, new_idx: int
    ) -> None:
        if task_hash not in self.data["tasks"]:
            return
        if step_hash not in self.data["tasks"][task_hash]["steps"]:
            return
    ...
    def change_order_tasks_group_insertion(
        self, group_name: str, task_hash: str, new_idx: int
    ) -> None:
        if group_name not in self.data["order of tasks in group"].keys():
            return
        if task_hash not in self.data["order of tasks in group"][group_name]:
            return
        current_index = self.data["order of tasks in group"][group_name].index(
            task_hash
        )
    ...
    def change_order_of_steps(self, task_hash: str, new_order: dict) -> None:
        if task_hash not in self.data["tasks"].keys():
            return
        for hash, value in new_order.items():
            if hash not in self.data["tasks"][task_hash]["steps"].keys():
                continue
    ...
    def delete_step(self, task_hash: str, step_hash: str) -> None:
        if task_hash not in self.data["tasks"].keys():
            return
        if step_hash not in self.data["tasks"][task_hash]["steps"].keys():
            return
        ...
        match type_of_idx_with_args[0]:
            case "standard":
                data = (
                    ...
                    if type_of_idx_with_args[1] is None
                    else type_of_idx_with_args[1].items()
                )
            ...
            case "group":
                try:
                    hash = self.data["order of tasks in group"][
                        type_of_idx_with_args[1]
                    ][idx - 1]
                except Exception:
                    pass
        return hash
    ...
    def get_hash_of_step_with_index(
        ...
        sorted_data = dict(
            sorted(
                self.data["tasks"][task_hash]["steps"].items(),
    ...
    def get_data_of_group(self, group_name: str) -> dict:
        dictionary = {}
        for key, values in self.data["tasks"].items():
            if group_name in values["groups"]:
                dictionary.update({key: values})
        return dictionary
    ...
    def get_data_of_importance(self, importance_lvl: str) -> dict:
        dictionary = {}
        for key, values in self.data["tasks"].items():
            if values["importance"] == importance_lvl:
                dictionary.update({key: values})
        return dictionary
    ...
    def display_task_details(self, data: dict, task_hash="") -> str:
        ...
        output = []
        if data == {}:
            return [["You did not create a task yet. Press A to change that.", "None"]]
        if self.current_order[0] == "group":
            index_map = {
                v: i
                for i, v in enumerate(
                    self.data["order of tasks in group"][self.current_order[1]]
                )
            }
        ...
        for idx, items in enumerate(data.items()):
            output.append(
                [
                    f"{idx+1}. {items[1]['name']} {''.join(f'[{label}]' for label in items[1]['labels'])}{''.join(f'{{{group}}}' for group in items[1]['groups'])}",
                    items[1]["importance"],
                ]
            )
            # display details
            if items[0] == task_hash:
                if items[1]["description"] != "":
                    output.append([items[1]["description"], "None"])
                output.append(["\n", "None"])
                # display steps with details
                sorted_steps = dict(
                    sorted(items[1]["steps"].items(), key=lambda item: item[1]["index"])
                )
                for idx_2, values in enumerate(sorted_steps.values()):
                    output.append(
                        [
                            f"{' '*self.TAB_INDENT}{idx+1}.{idx_2+1} {values['name']}",
                            values["importance"],
                        ]
                    )
                    if values["description"] != "":
                        output.append(
                            [f"{' '*self.TAB_INDENT}{values['description']}", "None"]
                        )
                    output.append(["\n", "None"])
        return output
    ...
    def get_longest_entry_beautified(self) -> tuple[int]:
        ...
        data = dict(
            sorted(self.data["tasks"].items(), key=lambda item: item[1]["index"])
        )
        for idx, items in enumerate(data.items()):
            output.append(
                f"{idx+1}. {items[1]['name']} {''.join(f'[{label}]' for label in items[1]['labels'])}{''.join(f'{{{group}}}' for group in items[1]['groups'])}"
            ...
            # display details
            if items[1]["description"] != "":
                output.append(items[1]["description"])
            output.append("\n")
            # display steps with details
            ...
            for idx_2, values in enumerate(sorted_steps.values()):
                ...
                if values["description"] != "":
                    output.append(f"{' '*self.TAB_INDENT}{values['description']}")
                output.append("\n")
            end_len = len(output) - start_len
            if end_len + number_of_tasks > min_height:
                min_height = end_len + number_of_tasks

        return (
            min_height + 2,
            len(max(output, key=lambda x: len(x))),
        )  # min_height + 2 bc of the outline top and bottom

    # Update and validate data automaticly
    def update_groups(self, task_hash: str, groups: list) -> None:
        for group in groups:
            try:
                if task_hash not in self.data["order of tasks in group"][group]:
                    self.data["order of tasks in group"][group].append(task_hash)
            except Exception:
                self.data["order of tasks in group"].update({group: [task_hash]})

    def validata_data_and_update_necessary(
        self, type_of_deletion_with_args: list[str]
    ) -> None:
        """
        ...
        match type_of_deletion_with_args[0]:
            case "task":
                sorted_data = dict(
                    sorted(
                        self.data["tasks"].items(), key=lambda item: item[1]["index"]
                    )
                )
            ...
            case "step":
                sorted_data = dict(
                    sorted(
                        self.data["tasks"][type_of_deletion_with_args[1][0]][
                            "steps"
                        ].items(),
                        key=lambda item: item[1]["index"],
                    )
