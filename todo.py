...
class ScreenManager:
    def __init__(self, file: str, content=None, content_beautified=None) -> None:
        ...
        self.windows = [
            self.screen,  # footer
            newpad(
                self.window_dimensions[1][0], self.window_dimensions[1][1]
            ),  # main todo
            newpad(self.window_dimensions[2][0], self.window_dimensions[2][1]),
        ]  # help message popup
        ...
        self.content = self.data.get_all_data() if content is None else content
        self.content_beautified = (
            self.data.display_task_details(self.content)
            if content_beautified is None
            else content_beautified
        )
        self.current_order_with_args = [
            "standard",
            self.content,
        ]  # other would be group, label and importance
    ...
    def event_handler(self, event: str) -> None:
        match event:
            ...
            case "KEY_RIGHT":
                if (
                    abs(self.scroll_y - self.window_dimensions[self.active_window][1])
            ...
            case "H" | "h":
                if self.active_window == 2:
                    ...
                    self.windows[1].refresh(
                        self.scroll_x,
                        self.scroll_y,
                        self.main_start_x_y[0],
                        self.main_start_x_y[1],
                        self.main_end_x_y[0],
                        self.main_end_x_y[1],
                    )
                else:
    ...
    def show_procedure(self) -> None:
        ...
        if index == "0":
            ...
            self.opened_task_hash = self.data.get_hash_of_task_with_index(
                int(index), self.current_order_with_args
            )
    ...
    def add_procedure(self) -> None:
        input = self.get_input_string(
            INSTRUCTIONS["add"]["1"], CHOICE_LEN, r"[TtSsLlGg]"
        )
        match input.lower():
            case "t":
                data = {}
                data["name"] = self.get_input_string(
                    INSTRUCTIONS["add"]["task"][0], NAME_LEN, NAME_REGEX
                )
                data["description"] = self.get_input_string(
                    INSTRUCTIONS["add"]["task"][1], DESCRIPTION_LEN
                )
                input = self.get_input_string(
                    INSTRUCTIONS["add"]["task"][2], CHOICE_LEN, r"[LlMmHhNn]"
                )
                match input.lower():
                    case "n":
                ...
                data["steps"] = {}
                input = self.get_input_string(
                    INSTRUCTIONS["add"]["task"][3][0], CHOICE_LEN, r"[EeNn]"
                )
                match input.lower():
                    case "e":
                        labels = self.data.get_labels()
                        index = self.get_input_string(
                            INSTRUCTIONS["add"]["task"][3][1] + labels[0],
                            MULTIINDEX_LEN,
                            MULTIINDEX_REGEX,
                        )
                        data["labels"] = self.get_all_possible_items(index, labels)
                    case "n":
                        name = self.get_input_string(
                            INSTRUCTIONS["add"]["task"][3][2], NAME_LEN, NAME_REGEX
                        )
                        data["labels"] = [name]
                input = self.get_input_string(
                    INSTRUCTIONS["add"]["task"][4][0], CHOICE_LEN, r"[EeNn]"
                )
                match input.lower():
                    case "e":
                        groups = self.data.get_groups()
                        index = self.get_input_string(
                            INSTRUCTIONS["add"]["task"][4][1] + groups[0],
                            MULTIINDEX_LEN,
                            MULTIINDEX_REGEX,
                        )
                    ...
                    case "n":
                        name = self.get_input_string(
                            INSTRUCTIONS["add"]["task"][4][2], NAME_LEN, NAME_REGEX
                        )
                        data["groups"] = [name]
                self.data.modify_task(data, True)
            case "s":
                data = {}
                idx = self.get_input_string(
                    INSTRUCTIONS["add"]["step"][0], INDEX_LEN, INDEX_REGEX
                )
                task_hash = self.data.get_hash_of_task_with_index(
                    int(idx), self.current_order_with_args
                )
                if not idx == "0" and task_hash != "":
                    data["name"] = self.get_input_string(
                        INSTRUCTIONS["add"]["step"][1], NAME_LEN, NAME_REGEX
                    )
                    data["description"] = self.get_input_string(
                        INSTRUCTIONS["add"]["step"][2], DESCRIPTION_LEN
                    )
                    input = self.get_input_string(
                        INSTRUCTIONS["add"]["step"][3], CHOICE_LEN, r"[LlMmHhNn]"
                    )
                    match input.lower():
                        case "n":
                            data["importance"] = "None"
    ...
    def change_procedure(self) -> None:
        input = self.get_input_string(
            INSTRUCTIONS["change"]["1"], CHOICE_LEN, r"[TtSsOoRr]"
        )
        match input.lower():
            case "t":
                input = self.get_input_string(
                    INSTRUCTIONS["change"]["task"]["1"],
                    MULTIINDEX_LEN,
                    MULTIINDEX_REGEX,
                )
                idx = [
                    int(i) - 1 for i in sorted(set(input.replace(" ", "").split(",")))
                ]
                task_idx = self.get_input_string(
                    INSTRUCTIONS["change"]["task"]["task_hash"], INDEX_LEN, INDEX_REGEX
                )
                task_hash = self.data.get_hash_of_task_with_index(
                    int(task_idx), self.current_order_with_args
                )
                if task_idx == "0" or task_hash == "":
                    return
                data = {}
                if 0 in idx:
                    data["name"] = self.get_input_string(
                        INSTRUCTIONS["change"]["task"]["2"][0], NAME_LEN, NAME_REGEX
                    )
                if 1 in idx:
                    data["description"] = self.get_input_string(
                        INSTRUCTIONS["change"]["task"]["2"][1], DESCRIPTION_LEN
                    )
                if 2 in idx:
                    input = self.get_input_string(
                        INSTRUCTIONS["change"]["task"]["2"][2],
                        CHOICE_LEN,
                        r"[LlMmHhNn]",
                    )
                    match input.lower():
                ...
                if 3 in idx:
                    input = self.get_input_string(
                        INSTRUCTIONS["change"]["task"]["2"][3][0], CHOICE_LEN, r"[EeNn]"
                    )
                    match input.lower():
                        case "e":
                            labels = self.data.get_labels()
                            index = self.get_input_string(
                                INSTRUCTIONS["change"]["task"]["2"][3][1] + labels[0],
                                MULTIINDEX_LEN,
                                MULTIINDEX_REGEX,
                            )
                            data["labels"] = self.get_all_possible_items(index, labels)
                        case "n":
                            name = self.get_input_string(
                                INSTRUCTIONS["change"]["task"]["2"][3][2],
                                NAME_LEN,
                                NAME_REGEX,
                            )
                ...
                if 4 in idx:
                    input = self.get_input_string(
                        INSTRUCTIONS["change"]["task"]["2"][4][0], CHOICE_LEN, r"[EeNn]"
                    )
                    match input.lower():
                        case "e":
                            groups = self.data.get_groups()
                            index = self.get_input_string(
                                INSTRUCTIONS["change"]["task"]["2"][4][1] + groups[0],
                                MULTIINDEX_LEN,
                                MULTIINDEX_REGEX,
                            )
                            data["groups"] = self.get_all_possible_items(index, groups)
                        case "n":
                            name = self.get_input_string(
                                INSTRUCTIONS["change"]["task"]["2"][4][2],
                                NAME_LEN,
                                NAME_REGEX,
                            )
            ...
            case "s":
                ...
                data = {}
                task_step_idx = self.get_input_string(
                    INSTRUCTIONS["change"]["step"]["2"][0], STEP_IDX_LEN, STEP_IDX_REGEX
                )
                task_hash, step_hash = self.data.get_hash_of_step_with_index(
                    task_step_idx, self.current_order_with_args
                )
                if not idx == "0" and task_hash != "" and step_hash != "":
                    if 0 in idx:
                        data["name"] = self.get_input_string(
                            INSTRUCTIONS["change"]["step"]["2"][1], NAME_LEN, NAME_REGEX
                        )
                    if 1 in idx:
                        data["description"] = self.get_input_string(
                            INSTRUCTIONS["change"]["step"]["2"][2], DESCRIPTION_LEN
                        )
                    if 2 in idx:
                        input = self.get_input_string(
                            INSTRUCTIONS["change"]["step"]["2"][3],
                            CHOICE_LEN,
                            r"[LlMmHhNn]",
                        )
                        match input.lower():
            ...
            case "o":
                input = self.get_input_string(
                    INSTRUCTIONS["change"]["order"]["what"], CHOICE_LEN, r"[TtSs]"
                )
                match input.lower():
                    case "t":
                        args = [
                            self.get_input_string(
                                INSTRUCTIONS["change"]["order"]["tasks"],
                                CHOICE_LEN,
                                r"[IiNn]",
                            )
                        ]
                    case "s":
                        index = self.get_input_string(
                            INSTRUCTIONS["change"]["order"]["steps"],
                            INDEX_LEN,
                            INDEX_REGEX,
                        )
                        task_hash = self.data.get_hash_of_task_with_index(
                        ...
                        if task_hash == "":
                            return
                        type_of_reorder = self.get_input_string(
                            INSTRUCTIONS["change"]["order"]["order_type"],
                            CHOICE_LEN,
                            r"[IiNn]",
                        )
                ...
                try:
                    ...
                    return
            case "r":
                type_of_value = self.get_input_string(
                    INSTRUCTIONS["change"]["rename"][0], CHOICE_LEN, r"[GgLl0]"
                )
                if type_of_value == "0":
                    return
                data = (
                    self.data.get_groups()
                    if type_of_value.lower() == "g"
                    else self.data.get_labels()
                )
                old_name = self.get_input_string(
                    INSTRUCTIONS["change"]["rename"][1] + data[0],
                    INDEX_LEN,
                    INDEX_REGEX,
                )
                if int(old_name) <= 0:
    ...
    def filter_procedure(self) -> None:
        input = self.get_input_string(
            INSTRUCTIONS["display"]["1"], CHOICE_LEN, r"[GgLlIi0]"
        )
        match input.lower():
            case "0":
                self.current_filter = ["self.data.get_all_data", ()]
            ...
            case "g":
                groups = self.data.get_groups()
                index = self.get_input_string(
                    INSTRUCTIONS["display"]["group"] + groups[0], INDEX_LEN, INDEX_REGEX
                )
                try:
                    if index == "0":
                        self.current_filter = [
                            "self.data.get_all_tasks_without",
                            ("groups"),
                        ]
                        self.current_order_with_args = ["standard", self.content]
                        self.data.change_current_order_to(self.current_order_with_args)
                        ...
                        self.current_filter = [
                            "self.data.get_data_of_group",
                            (
                                list(self.data.data["order of tasks in group"].keys())[
                                    int(index) - 1
                                ]
                            ),
                        ]
            ...
            case "l":
                labels = self.data.get_labels()
                index = self.get_input_string(
                    INSTRUCTIONS["display"]["label"] + labels[0], INDEX_LEN, INDEX_REGEX
                )
                try:
                    if index == "0":
                        self.current_filter = [
                        ...
                        self.current_filter = [
                            "self.data.get_data_with_label",
                            (labels[1][int(index) - 1]),
                        ]
                        self.update_content(
                            self.data.get_data_with_label(labels[1][int(index) - 1])
                        )
                        self.current_order_with_args = ["standard", self.content]
                except Exception:
                    pass
            ...
            case "i":
                input = self.get_input_string(
                    INSTRUCTIONS["display"]["importance"], CHOICE_LEN, r"[LlMmHhNn]"
                )
                match input.lower():
                    case "n":
                        self.current_filter = [
                            "self.data.get_data_of_importance",
                            ("None"),
                        ]
    ...
    def delete_procedure(self) -> None:
        ...
        input = self.get_input_string(
            INSTRUCTIONS["delete"]["1"], CHOICE_LEN, r"[TtSsLlGg]"
        )
        match input.lower():
            case "t":
                index = self.get_input_string(
            ...
            case "s":
                ...
                data_to_delete = [
                    ...
                    self.data.get_hash_of_step_with_index(
                        index, self.current_order_with_args
                    ),
                ]
            case "l":
                labels = self.data.get_labels()
    ...
    def call_order_methods(self, items_to_reorder: str, args: list[str]) -> None:
        match args[0].lower():
            case "i":
                input = self.get_input_string(
                    INSTRUCTIONS["change"]["order"]["insert"],
                    MULTIINDEX_LEN,
                    r"^\d+,\s*\d+$",
                )
                ...
                match items_to_reorder:
                    case "t":
                        task_hash = self.data.get_hash_of_task_with_index(
                            int(old_idx), self.current_order_with_args
                        )
                        if self.current_order_with_args[0] == "group":
                            self.data.change_order_tasks_group_insertion(
                                self.current_order_with_args[1],
                                task_hash,
                                int(new_idx) - 1,
                            )
                        elif self.current_order_with_args[0] == "standard":
                            self.data.change_order_tasks_global_insertion(
                                task_hash, int(new_idx) - 1
                            )
                    case "s":
                        ...
                        if args[1] == "" or step_hash == "":
                            return
                        self.data.change_order_of_steps_insertion(
                            args[1], step_hash, int(new_idx) - 1
                        )
            case "n":
                ...
                match items_to_reorder:
                    case "t":
                        if self.current_order_with_args[0] == "group":
                            self.data.change_group_order_of_tasks(
                                self.current_order_with_args[1], new_order
                            )
                        elif self.current_order_with_args[0] == "standard":
    ...
    def get_input_string(self, p_message: str, input_length: int, p_regex=None) -> str:
        ...
        while True:
            ...
            win = newwin(
                self.main_end_x_y[0] - 1,
                self.main_end_x_y[1] + 1,
                self.main_start_x_y[0],
                self.main_start_x_y[1],
            )
            echo()
            ...
            win.box()
            win.refresh()
            input = win.getstr(height_of_msg, 1, input_length).decode(
            ...
            if match(regex, input):
                ...
                message = self.make_message_fit_width(
                    WRONG_INPUT_MESSAGE + p_message, self.window_dimensions[0][1] - 2
                )
                height_of_msg = len(message.splitlines()) + 1
        return input
    ...
    def get_input_new_order(
        ...
        while True:
            ...
            if match(r"^\d+(,\s*\d+)*$", input):
                ...
                all_indices_given = all(
                    i in range(len(content) + 1) for i in new_order_idx
                )
            ...
            if all_indices_given:
                ...
                message_is_updated = True
                message = self.make_message_fit_width(
                    "You need to provide all indices and separate them by a `,`!!!\n\n"
                    + p_message,
                    self.window_dimensions[0][1] - 2,
                )
                height_of_msg = len(message.splitlines()) + 1
        ...
        if items_to_reorder == "t" and self.current_order_with_args[0] == "group":
            new_order = [
                self.data.get_hash_of_task_with_index(
                    idx + 1, self.current_order_with_args
                )
                for idx in new_order_idx
            ]
        elif items_to_reorder == "t" and self.current_order_with_args[0] == "standard":
            new_order = {
                self.data.get_hash_of_task_with_index(
                    i + 1, self.current_order_with_args
                ): idx
    ...
    def update_content(self, content: dict) -> None:
        ...
        self.content_beautified = self.data.display_task_details(
            content, self.opened_task_hash
        )
    ...
    def beautify_output(self, start_y=1, start_x=1) -> None:
        ...
        for line, importance in self.content_beautified:
            if importance != "None":
                self.output_text_to_window(
                    1, line, start_y, start_x, self.__dict__[f"{importance}_importance"]
                )
            else:
